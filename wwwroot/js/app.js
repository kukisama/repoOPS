...
// ---- Runtime wwwroot compatibility layer ----
// The published/root wwwroot historically shipped without i18n/config-editor.
// If those scripts are present, we use them. If not, provide a safe fallback.
...
async function loadConfig() {
    try {
        ...
        document.getElementById('taskList').innerHTML =
            '<div class="loading">Failed to load tasks. Check tasks.json configuration.</div>';
    }
}
...
function renderTaskList(config) {
    ...
    if (!config.groups || config.groups.length === 0) {
        container.innerHTML = '<div class="loading">No tasks configured. Edit tasks.json to add tasks.</div>';
        return;
    }
...
async function closeTab(executionId) {
    ...
    if (activeTabId === executionId) {
        ...
        if (remainingTabs.length > 0) {
            ...
            const tabBar = document.getElementById('tabBar');
            tabBar.innerHTML = '<div class="tab-placeholder">Select a task to run</div>';
        }
    }
